...
public class Upload {
    ...
    public boolean deleteImage(String directoryPath, String fileName) {
        String[] path = fileName.split("/");
        File file = new File(directoryPath + File.separator + path[path.length - 1]);
        return file.delete();
    }

    public boolean deleteImageDescription(String directoryPath, List<ImgDescription> imgs) {
        boolean isDelete = false;
        for (ImgDescription img : imgs) {
            String[] path = img.getImgUrl().split("/");
            File file = new File(directoryPath + File.separator + path[path.length - 1]);
    ...
    public boolean deleteImageColor(String directoryPath, List<Color> colors) {
        boolean isDelete = false;
        for (Color color : colors) {
            String[] path = color.getImgColor().split("/");
            File file = new File(directoryPath + File.separator + path[path.length - 1]);
            isDelete = file.delete();
        }
        return isDelete;
