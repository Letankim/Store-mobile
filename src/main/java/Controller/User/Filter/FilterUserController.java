    ...
    private String filterProduct(List<Product> products) {
        ...
        if (products.size() == 0) {
            productReturn = "<div class=\"box-no-found\">\n"
                    ...
                    + "                            <p class=\"text\">Sorry, No Product Found</p>\n"
                    + "                          </div>";
            return productReturn;
        }
        for (Product product : products) {
            productReturn += "<div class=\"card col-6 col-lg-3 cart-filter\"  onclick=\"changePage(this, event)\">\n"
                    + "                                            <div class=\"box-img\">\n"
            ...
            if (product.getNewPrice() > 0) {
                productReturn += "<span class=\"new-price\">" + currency.currencyFormat(product.getNewPrice(), "$") + "</span>\n"
                        + "<span class=\"old-price\">" + currency.currencyFormat(product.getOldPrice(), "$") + "</span>\n"
                        + "                                                </p>\n";
            } else {
                productReturn += "<span>" + currency.currencyFormat(product.getOldPrice(), "$") + "</span>\n</p>\n";
            }
